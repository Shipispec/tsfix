 ...
 *
 * The architectural moat for `@shipispec/tsmend`. When a tsc diagnostic says
 * "Property 'foo' doesn't exist on type 'Bar'", this resolves the `Bar`
 ...
 *
 * Mirrors the lib-path workaround pattern from
 * @shipispec/tsfix/src/validatorInProcess.ts:172.
 */

import * as fs from "node:fs";
import * as path from "node:path";
import * as ts from "typescript";
import type { Diagnostic } from "@shipispec/tsfix";
...
function buildProgram(workspaceRoot: string): ts.Program | null {
	...
	// Lib-path workaround — same as @shipispec/tsfix/validatorInProcess.ts:172.
