---
title: "tsc-correctness ≠ runtime-correctness"
published: false
description: "An LLM's tsc fix can pass the compiler and break your dev server — or worse, ship an unsalted password hash. Here's the gap, and the open-source way to close it."
tags: typescript, ai, webdev, opensource
# cover_image: https://your-image-url-1000x420.png
# canonical_url: leave unset so dev.to is the canonical source for SEO
---
...
tsfix reads your `package.json` on every Layer-2 invocation. If it sees `vite-plugin-svgr` at v4 or higher in your dependencies, it injects this into the system prompt headline, *before* the model sees the errored file:
